  ...
  <head>
    ...
    <style>
      ...
      .slide-in-from-bottom-4 {
        animation: slideInBottom4 0.7s ease-out forwards;
      }
  ...
  <body>
    ...
    <script type="text/babel" data-type="module">
      ...
      const checkSlipStatus = async (slip) => {
        try {
          const ai = new GoogleGenAI({ apiKey: API_KEY });
          const prompt = `Verifique resultados finais dos jogos: ${JSON.stringify(
            slip.selections.map((s) => ({
              match: s.match,
              selection: s.selection,
            }))
          )}. Retorne JSON array strings: ["won", "lost", "pending"].`;
          const response = await ai.models.generateContent({
            model: 'gemini-2.5-flash',
            contents: prompt,
      ...
      const Header = ({ notificationsEnabled, onToggle }) => (
        <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md shadow-md border-b-4 border-green-700">
          <div className="max-w-4xl mx-auto px-4 py-4 flex justify-between items-center">
            ...
            <button
              onClick={onToggle}
              className={`p-2 rounded-full shadow-sm border border-slate-100 ${
            ...
            >
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center space-x-2 text-sm text-slate-600 font-medium">
                  <span>
                    {selection.sport && selection.sport.includes('Fut')
                      ? '⚽'
                      : '🏀'}
                  </span>
              ...
              <div className="px-4 pb-4 pt-0 bg-slate-50/80 border-t border-slate-100">
                <div className="mt-3">
                  <h4 className="text-xs font-bold text-red-600 uppercase mb-2 flex items-center">
                    <BarChart2 size={12} className="mr-1" /> Forma (Últimos 5
                    Jogos)
                  </h4>
      ...
      const HistoryList = ({ history, onCheck, checkingId }) => (
        <div className="space-y-4">
          {history.length === 0 ? (
            <div className="text-center py-8 text-slate-500 text-sm bg-white/60 backdrop-blur-sm rounded-xl">
            ...
            history.map((slip) => (
              <div
                key={slip.id}
                className={`relative rounded-xl border-l-4 overflow-hidden shadow-sm backdrop-blur-sm ${
                  slip.status === 'won'
                ...
                <div className="p-4">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <div className="text-xs text-slate-500 uppercase font-semibold">
                        {new Date(slip.date).toLocaleDateString('pt-PT')}
                      </div>
                  ...
                  <div className="space-y-2 mb-3">
                    {slip.selections.map((sel, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-center text-sm border-b border-black/5 pb-1 last:border-0"
                      >
                        <span className="truncate max-w-[60%] text-slate-700">
                          {sel.match}
                        </span>
                        <div className="flex items-center space-x-2">
                  ...
                  {slip.status === 'pending' && (
                    <button
                      onClick={() => onCheck(slip.id)}
                      disabled={checkingId === slip.id}
                    ...
                    >
                      {checkingId === slip.id ? (
                        ...
                        <SearchCheck size={14} className="text-blue-600" />
                      )}
                      <span>
                        {checkingId === slip.id
                          ? 'Verificando...'
      ...
      const App = () => {
        ...
        const runLogic = useCallback(async () => {
          ...
          if (storedDataStr) {
            ...
            localHistory = localHistory.filter(
              (s) => new Date(s.date) > tenDaysAgo
            );

            if (localSlip) {
              const slipDate = new Date(localSlip.date).toLocaleDateString(
                'pt-PT'
              );
              // PERSISTÊNCIA RÍGIDA: Se tem slip de hoje, usa-o e pronto.
              if (slipDate === todayStr) {
                setAppStatus('success');
                setCurrentSlip(localSlip);
                setHistory(localHistory);
                return;
              }
              if (slipDate !== todayStr) {
                if (!localHistory.find((h) => h.id === localSlip.id))
                  localHistory.unshift(localSlip);
                localSlip = null;
              }
            }
          }
          ...
          if (newSlip) {
            ...
            localStorage.setItem(
              STORAGE_KEY,
              JSON.stringify({ history: localHistory, currentSlip: newSlip })
            );
          } else {
            setAppStatus('error');
          }
        ...
        const handleCheckHistory = async (id) => {
          ...
          if (slip) {
            ...
            localStorage.setItem(
              STORAGE_KEY,
              JSON.stringify({ history: newHistory, currentSlip })
            );
          }
          setCheckingId(null);
        };
                ...
                <div className="animate-fade-in space-y-6">
                  ...
                  {appStatus === 'error' && (
                    <div className="text-center py-12 bg-white/90 backdrop-blur-sm rounded-xl shadow border border-red-100">
                      ...
                      <button
                        onClick={() => runLogic()}
                        className="mt-4 bg-red-600 text-white px-6 py-2 rounded-full font-bold shadow hover:bg-red-700 transition"
                      >
                        Recarregar
                      </button>
                    </div>
                  ...
                  {appStatus === 'success' && currentSlip && (
                    <>
                        ...
                        <div className="relative z-10">
                          <div className="flex items-center space-x-2 mb-1">
                            ...
                            <p className="text-green-100 text-xs">
                              {new Date(currentSlip.date).toLocaleDateString(
                                'pt-PT'
                              )}
                            </p>
                          </div>
                          <h2 className="text-3xl font-bold text-white">
                            ...
                            <span className="text-yellow-300">
                              {currentSlip.totalOdd.toFixed(2)}
                            </span>
                          </h2>
                        </div>
                      ...
                      >
                        {currentSlip.selections.map((sel, idx) => (
                          <BetCard key={idx} selection={sel} />
                        ))}
                      </div>
                      <div className="text-center p-4 bg-yellow-50/90 backdrop-blur-sm rounded-lg border border-yellow-100 text-xs text-yellow-900 font-bold shadow-sm">
                        ...
                        Gestão recomendada: 1-2% da banca.
                      </div>
                    </>
                  )}
                </div>
              ...
              {activeTab === 'history' && (
                <div className="bg-white/90 backdrop-blur-md rounded-xl shadow-sm border border-slate-200 p-4 animate-fade-in">
                  <h2 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
                    <History className="text-red-600" size={20} /> Histórico (10
                    Dias)
                  </h2>
                  <HistoryList
                    history={history}
                    onCheck={handleCheckHistory}
